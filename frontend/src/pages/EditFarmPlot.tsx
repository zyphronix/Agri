...
import { getFarmPlot, updateFarmPlot, detectLocation } from '@/services/farmService';
...
export default function EditFarmPlot() {
  const { t } = useLanguage();
  ...
  useEffect(() => {
    const loadPlot = async () => {
      ...
      try {
        const plot = await getFarmPlot(id);
        if (plot) {
          setFormData({
            name: plot.name,
        ...
        console.error('Failed to load farm plot:', error);
        toast.error('Failed to load farm plot');
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.area || !id) {
    ...
    try {
      ...
      console.error('Failed to update farm plot:', error);
    ...
    <div className="min-h-screen bg-background pb-6">
      <div className="bg-primary/5 pt-8 pb-6 px-4">
        <div className="max-w-2xl mx-auto">
          ...
          <h1 className="text-2xl font-bold text-foreground">{t('farm.editPlot')}</h1>
        </div>
      </div>
